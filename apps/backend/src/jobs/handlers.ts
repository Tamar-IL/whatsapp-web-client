...
export async function registerHandlers(): Promise<void> {
  ...
  await boss.work<JobPayloads['download-media']>('download-media', async ([job]) => {
  ...
  await boss.work<JobPayloads['send-media']>('send-media', async ([job]) => {
    logger.info({ jobId: job.id, data: job.data }, 'send-media stub');
    // TODO Phase 4 ticket 4.3
  });

  await boss.work<JobPayloads['send-voice']>('send-voice', async ([job]) => {
  ...
  await boss.work<JobPayloads['reconcile-status']>('reconcile-status', async ([job]) => {
